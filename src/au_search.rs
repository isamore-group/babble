  ...
  Op: Clone + Debug + Hash + Ord + Display + Arity + Teachable + Sync + Send + 'static,
{
    ...
    // // 将点集中size()>400的点全部删掉
    // let mut candidates = candidates.into_iter().filter(|x| x.exprs.iter().map(|x| x.size()).sum::<usize>() < 400).collect::<BTreeSet<_>>();
    // // 如果candidates的size小于m, 直接返回candidates
    // if candidates.len() < m {
...
pub fn kd_random_aus<Op>(
    ...
    Op: Clone + Debug + Hash + Ord + Display + Arity + Teachable + Sync + Send + 'static,
  {
    ...
    let candidates = if cal_cartesian_flag {
        ...
        // // 维护一个优先队列，存储维度i和aus[i]切分后当前的长度，这个优先队列根据长度从大到小进行排序
        ...
        //     cnt += 1;
        // }
        // // 如果cnt < depth, 那么说明有集合的长度为1，那么剩下的切分次数平摊到每个集合
        // if cnt < depth {
...
// 一个小测试，因为实验中发现好像采样方法改变好像对于结果影响不大，所以此处写一个greedy_aus,对于给定的aus，只只取最大值和最小值
