  ...
  LD: LangGain<Op> + Clone + Default,
{
          ...
          Some(cand) => {
            ...
            if let Some(index) = self.get_from_memo(id) {
              ...
              if let Some(prev) = self.all_exprs[*index].clone() {
                // 存在表达式，首先计算prev的cost，如果all_costs中有值就不用计算，直接取出
                ...
                // 如果prev_cost < c_cost，说明cand的cost更大, 将flag置为false,否则就将c_cost赋给cand_cost
                if prev_cost < c_cost {
                  flag = false;
                } else {
  ...
  fn extract_children(
    ...
    if current == node.children().len() {
      ...
      println!("using {}ms to extract children", child_start.elapsed().as_millis());
      ...
      match child_res {
        ...
        Some(expr) => {
          ...
          let exp = self.extract_children(node, current + 1, partial_expr, child_indexes);
