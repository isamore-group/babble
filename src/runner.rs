    ...
    LD: LangGain<Op> + Clone + Send + Sync + 'static,
{
    ...
    /// Run the e-graph and library learning process with Pareto-optimal beam search
    fn run_egraph(
        &self,
        roots: &[Id],
        ...
        debug!("area cost: {}, delay cost: {}", cs.set[0].full_area_cost, cs.set[0].full_delay_cost);
    ...
    LD: LangGain<Op> + Clone + Default,
{
    ...
    /// Run the experiment on groups of equivalent expressions
    fn run_equiv_groups(&self, expr_groups: Vec<Vec<Expr<Op>>>) -> KnapsackResult<Op, LA, LD>;
}
    ...
    LD: LangGain<Op> + Clone + Default,
{
    ...
    pub num_libs: usize,
    /// The rewrites representing the learned libraries
    pub rewrites: Vec<Rewrite<AstNode<Op>, beam_knapsack::PartialLibCost<Op, LA, LD>>>,
    /// The initial cost of the expression(s)
    pub initial_cost: usize,
    /// The final cost of the expression
    pub final_cost: usize,
...
where
    Op: Display + Hash + Clone + Ord + Teachable + Arity + Send + Sync + Debug + 'static,
    LA: LangCost<Op> + Clone + Default,
    LD: LangGain<Op> + Clone + Default,
{
    /// The domain-specific rewrites to apply
    ...
    LD: LangGain<Op> + Clone + Default,
{
    ...
    fn run_egraph(
        ...
        let runner = EggRunner::<_, _, ()>::new(beam_knapsack::PartialLibCost::empty())
            .with_egraph(egraph)
            .with_time_limit(timeout)
            .with_iter_limit(3)
            .run(&self.dsrs);
        ...
        info!("Adding libs and running beam search... ");
        let lib_rewrite_time = Instant::now();
        let runner = EggRunner::<_, _, ()>::new(beam_knapsack::PartialLibCost::new(
            self.config.final_beams,
            self.config.inter_beams,
            self.config.lps,
